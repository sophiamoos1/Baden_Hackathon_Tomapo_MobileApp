...
struct TomapoStationDetailView: View {
            ...
            VStack(spacing: 0) {
                ...
                if station.isVerified {
                    ...
                    DetailRow(label: "Verifiziert", value: "Nicht verifiziert",
                              valueColor: Color.theme.secondaryText.opacity(0.4))
                }
    ...
    private var statusColor: Color {
        ...
        case .pending:    return Color.theme.secondaryText.opacity(0.5)
        case .skipped:    return Color.theme.secondaryText.opacity(0.3)
        case .unknown:    return Color.theme.secondaryText.opacity(0.35)
        }
    }
...
private struct FishingDetailSection: View {
    ...
    var body: some View {
        DetailCardSection(title: "Fischfang / Aquakultur", icon: "fish.fill") {
            ...
            InfoDetailRow(label: "MSC-zertifiziert", value: detail.isMscCertified ? "Ja" : "Nein", valueColor: detail.isMscCertified ? .green : Color.theme.secondaryText.opacity(0.5))
            InfoDetailRow(label: "ASC-zertifiziert", value: detail.isAscCertified ? "Ja" : "Nein", valueColor: detail.isAscCertified ? .green : Color.theme.secondaryText.opacity(0.5))
...
private struct ProcessingDetailSection: View {
    ...
    var body: some View {
        DetailCardSection(title: "Verarbeitung", icon: "gearshape.fill") {
            ...
            InfoDetailRow(label: "HACCP", value: detail.isHaccpCertified ? "Zertifiziert" : "Nicht zertifiziert", valueColor: detail.isHaccpCertified ? .green : Color.theme.secondaryText.opacity(0.5))
...
private struct DistributionDetailSection: View {
    ...
    var body: some View {
        DetailCardSection(title: "Distribution / Verteilzentrum", icon: "building.2") {
            ...
            InfoDetailRow(label: "Zollabgefertigt", value: detail.customsCleared ? "Ja" : "Nein", valueColor: detail.customsCleared ? .green : Color.theme.secondaryText.opacity(0.5))
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            DetailCardSection(title: "Labor", icon: "flask.fill") {
                ...
                InfoDetailRow(label: "ISO 17025", value: detail.isIso17025Accredited ? "Akkreditiert ✓" : "Nicht akkreditiert", valueColor: detail.isIso17025Accredited ? .green : Color.theme.secondaryText.opacity(0.5))
                        ...
                        HStack {
                            VStack(alignment: .leading, spacing: 2) {
                                ...
                                if let notes = r.notes {
                                    Text(notes).font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.5)).lineLimit(2)
                                }
                            }
                            Spacer()
                            Text("\(String(format: "%.2f", r.measuredValue)) \(r.unit)")
                                .font(.subheadline)
                                .foregroundColor(r.isWithinSpec ? Color.theme.secondaryText : Color.theme.error)
                        }
                        .padding(.horizontal, 16).padding(.vertical, 10)
                    }
                }
                        ...
                        HStack(alignment: .top, spacing: 10) {
                            ...
                            VStack(alignment: .leading, spacing: 2) {
                                Text(r.claim)
                                    .font(.subheadline)
                                    .foregroundColor(Color.theme.secondaryText)
                                if let method = r.method { Text(method).font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.5)) }
                                if let conf = r.confidencePercent { Text("Konfidenz: \(String(format: "%.1f", conf))%").font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.5)) }
                                if let notes = r.notes { Text(notes).font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.45)).lineLimit(3) }
                            }
                        }
                        .padding(.horizontal, 16).padding(.vertical, 10)
                    }
...
private struct QualityCheckCard: View {
    ...
    private var statusColor: Color {
        ...
        case .failed: return Color.theme.error
        case .warning: return Color.theme.warning
        default: return Color.theme.secondaryText.opacity(0.4)
        }
    }
...
private struct QualityDetailContent: View {
    ...
    var body: some View {
        ...
        case .chemical(let d):
            VStack(alignment: .leading, spacing: 4) {
                ForEach(d.substancesTested, id: \.substance) { s in
                    HStack(spacing: 6) {
                        Circle()
                            .fill(s.limitExceeded ? Color.theme.error : (s.measuredMgPerKg != nil ? .green : Color.theme.secondaryText.opacity(0.3)))
                            .frame(width: 6, height: 6)
                        Text(s.substance)
                            .font(.caption2)
                            .foregroundColor(Color.theme.secondaryText.opacity(0.65))
                        ...
                        if let v = s.measuredMgPerKg {
                            Text("\(String(format: "%.4f", v)) \(s.unit)")
                                .font(.caption2)
                                .foregroundColor(s.limitExceeded ? Color.theme.error : Color.theme.secondaryText.opacity(0.55))
                        } else {
        ...
        case .nutritional(let d):
            HStack(spacing: 16) {
                if let e = d.energyKcal { VStack { Text("\(Int(e))").font(.caption.weight(.bold)).foregroundColor(Color.theme.secondaryText); Text("kcal").font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.5)) } }
                if let f = d.fatG { VStack { Text("\(String(format: "%.1f", f))g").font(.caption.weight(.bold)).foregroundColor(Color.theme.secondaryText); Text("Fett").font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.5)) } }
                if let p = d.proteinsG { VStack { Text("\(String(format: "%.1f", p))g").font(.caption.weight(.bold)).foregroundColor(Color.theme.secondaryText); Text("Protein").font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.5)) } }
                if let dev = d.deviationFromLabelPercent {
                    VStack {
                        ...
                        Text("Abw.").font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.5))
                    }
                }
            }
        ...
        case .visual(let d):
            HStack(spacing: 8) {
                if let grade = d.gradeAssigned { Text(grade).font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.65)) }
                if let rej = d.rejectionRate { Text("Ausschuss: \(String(format: "%.1f", rej))%").font(.caption2).foregroundColor(rej < 5 ? Color.theme.secondaryText.opacity(0.55) : Color.theme.warning) }
            }

        case .weight(let d):
            HStack(spacing: 16) {
                VStack { Text("\(String(format: "%.1f", d.targetWeightG))g").font(.caption.weight(.bold)).foregroundColor(Color.theme.secondaryText); Text("Soll").font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.5)) }
                VStack { Text("\(String(format: "%.1f", d.measuredWeightG))g").font(.caption.weight(.bold)).foregroundColor(d.isWithinTolerance ? Color.theme.secondaryText : Color.theme.error); Text("Ist").font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.5)) }
                VStack { Text("±\(String(format: "%.1f", d.tolerancePercent))%").font(.caption.weight(.bold)).foregroundColor(Color.theme.secondaryText); Text("Toleranz").font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.5)) }
            }

        case .certification(let d):
            VStack(alignment: .leading, spacing: 4) {
                HStack {
                    Text(d.certificationBody).font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.65))
                    Spacer()
                ...
                if let valid = d.certificateValidUntil {
                    Text("Gültig bis \(valid.formatted(.dateTime.day().month().year()))")
                        .font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.45))
                }
            }

        case .generic(let d):
            if let desc = d.description { Text(desc).font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.65)).lineLimit(3) }
        }
    }
}
            ...
            VStack(alignment: .trailing, spacing: 2) {
                if let v = result.measuredValue {
                    Text("\(String(format: "%.4f", v)) \(result.unit)")
                        .font(.caption.weight(.semibold))
                        .foregroundColor(result.mrlExceeded ? Color.theme.error : Color.theme.secondaryText)
                } else {
                    Text("n.d.")
                        .font(.caption.weight(.semibold))
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            VStack(alignment: .trailing, spacing: 1) {
                if let name = location.name { Text(name).font(.caption).foregroundColor(Color.theme.secondaryText) }
                ...
                if !parts.isEmpty { Text(parts.joined(separator: ", ")).font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.5)) }
                if let emb = location.embCode { Text("EMB: \(emb)").font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.4)) }
                if let gln = location.gln { Text("GLN: \(gln)").font(.caption2).foregroundColor(Color.theme.secondaryText.opacity(0.35)) }
            }
        }
        .padding(.horizontal, 16).padding(.vertical, 11)
    }
