...
struct ContentView: View {
    ...
    @StateObject private var userStore          = TomapoUserStore()
    @StateObject private var userMessageStore   = TomapoUserMessageStore()
    ...
    var body: some View {
        ZStack {
            ...
            .safeAreaInset(edge: .bottom, spacing: 0) {
                BottomBar(selectedTab: $selectedTab).background(.clear)
            }
 
            // MARK: Screen-Level Popup
            if let item = popupManager.activeItem {
                CardDetailsPopup(item: item) {
                    withAnimation(.easeInOut(duration: 0.22)) { popupManager.dismiss() }
                }
                .ignoresSafeArea()
